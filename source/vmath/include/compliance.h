...
*/
    ...
    /**
     * \brief Calculate Airway Resistance (Raw)
     * 
     * In the ventilated patient, resistance can be measured by dividing the 
     * [peak pressure minus the plateau pressure] by the flowrate in litres per second.
     * 
     * The equation is:
     * \f[
     *   Raw = \frac{(PIP - PEEP)}{Lps}\\
     * \f]
     * 
     * 
     */
    inline double calculate_Raw(double Pplat, double Ppeak, double liters_per_second)
    {
        return ( (Ppeak - Pplat ) / liters_per_second );
    }
} // math
} // ovp
